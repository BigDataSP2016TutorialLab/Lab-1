...
object SparkMovieRecommendation {
  def main(args: Array[String]) {
    ...
    val movies = sc.textFile(new File(movieLensHomeDir, "twitter.dat").toString).map { line =>
      val fields = line.split("::")
    ...
    SocketClient.sendCommandToRobot("Got " + numRatings + " ratings from "
      + numUsers + " users on " + numMovies + " tweets.")
    ...
    var i = 1
    SocketClient.sendCommandToRobot("Tweets recommended for you:\n")
...
object SocketClient {
  ...
  def sendCommandToRobot(string: String)
  {
    try {
      ...
      lazy val address: Array[Byte] = Array(192.toByte, 168.toByte, 1.toByte, 141.toByte)
